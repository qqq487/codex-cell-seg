...
def follow_flows(dP, mask=None, inds=None, niter=200, interp=True, use_gpu=True, device=None, omni=False, calc_trace=False):
    ...
    if len(shape)>2: ## No!
        ...
        if inds.ndim < 2 or inds.shape[0] < 5: ## NO!
...
def compute_masks(dP, cellprob, bd=None, p=None, inds=None, niter=200, mask_threshold=0.0, diam_threshold=12.,
    ...
    # print("before any, cp_mask.shape = ",cp_mask.shape)
    # print("before any, cp_mask.dtype = ",cp_mask.dtype)
    # print("before any, cp_mask = ",cp_mask)

    if np.any(cp_mask): #mask at this point is a cell cluster binary map, not labels     
        # follow flows
        if p is None:
            
            p , inds, tr = follow_flows(dP * cp_mask / 5., mask=cp_mask, inds=inds, niter=niter, interp=interp, use_gpu=use_gpu, device=device)
            if inds.ndim < 2 or inds.shape[0] < 5:  ## NO!
